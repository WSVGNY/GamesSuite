...
import { RED, PI_OVER_4, PI_OVER_2, HALF_TRACK_WIDTH } from "../../constants";
...
export class ConstraintValidator {
    ...
    public static checkLength(connections: Array<Line>): boolean {
        ...
        for (const connection of connections) {
            ...
            if (this.calculateLength(geometry.vertices) < HALF_TRACK_WIDTH) {
    ...
    public static checkIntersection(connections: Array<Line>, isComplete: boolean): boolean {
        ...
        for (let i: number = 0; i < connections.length; i++) {
            ...
            for (let j: number = 0; j < limit; j++) {
                if (j > i + 1) {
                    const vectors1: Array<Vector3[]> = this.generateTrackWidth(connections[i], HALF_TRACK_WIDTH);
                    const vectors2: Array<Vector3[]> = this.generateTrackWidth(connections[j], HALF_TRACK_WIDTH);
