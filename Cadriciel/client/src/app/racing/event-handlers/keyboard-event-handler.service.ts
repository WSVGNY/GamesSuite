...
export class KeyboardEventHandlerService {
    ...
    public handleKeyDown(event: KeyboardEvent, car: Car): void {
        switch (event.keyCode) {
            case ACCELERATE_KEYCODE:
                this._carControl.command = new GoFoward(car);
                this._carControl.execute();
                break;
            case LEFT_KEYCODE:
                this._carControl.command = new TurnLeft(car);
                this._carControl.execute();
                break;
            case RIGHT_KEYCODE:
                this._carControl.command = new TurnRight(car);
                this._carControl.execute();
                break;
            case BRAKE_KEYCODE:
                this._carControl.command = new Brake(car);
                this._carControl.execute();
                break;
            // case DAY_KEYCODE:
            //     raceGame.isDay = true;
    ...
    public handleKeyUp(event: KeyboardEvent, car: Car): void {
        switch (event.keyCode) {
            case ACCELERATE_KEYCODE:
                this._carControl.command = new ReleaseAccelerator(car);
                this._carControl.execute();
                break;
            case LEFT_KEYCODE:
            case RIGHT_KEYCODE:
                this._carControl.command = new ReleaseSteering(car);
                this._carControl.execute();
                break;
            case BRAKE_KEYCODE:
                this._carControl.command = new ReleaseBrakes(car);
                this._carControl.execute();
                break;
            default:
                break;
