...
export class CollisionManagerService {
                ...
                if (cars[i].currentPosition.distanceTo(cars[j].currentPosition) < MINIMUM_CAR_DISTANCE) {
                    ...
                    for (let vertexIndex: number = 0; vertexIndex < cars[i].hitbox.vertices.length; ++vertexIndex) {
                        ...
                        if (collisionResult.length > 0 && collisionResult[0].distance < direction.length()) {
                            ...
                            // this.computeResultingForces(cars[j], cars[i], collisionResult[0].point);
                            ...
                            this.collisionAxisHelper.update(collisionResult[0].point.clone().add(new Vector3(0, 0.2, 0)),
    ...
    private computeCollisionAxis(firstCar: Car, secondCar: Car, collisionPoint: Vector3): Vector3 {
        ...
        const halfOfSmallAngle: number = collisionToFirstCar.angleTo(collisionToSecondCar) / 2;

        return collisionToFirstCar.clone().normalize().applyAxisAngle(new Vector3(0, 1, 0), halfOfSmallAngle);
    }
